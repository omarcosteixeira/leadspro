...
export function AdminFuncionariosView({ onToast }: Props) {
  ...
  const handleDeleteFuncionario = async (id: string, name: string) => {
    if (!window.confirm(`Tem certeza que deseja remover o funcionário ${name}?`)) {
      return;
    }
    try {
  ...
  const handleExportExcel = () => {
    ...
    const max_len = dataToExport.reduce((acc, row) => {
      acc['Nome Completo'] = Math.max(acc['Nome Completo'] || 0, String(row['Nome Completo']).length);
      acc['E-mail'] = Math.max(acc['E-mail'] || 0, String(row['E-mail']).length);
      acc['Tipo'] = Math.max(acc['Tipo'] || 0, String(row['Tipo']).length);
      acc['Matrícula'] = Math.max(acc['Matrícula'] || 0, String(row['Matrícula']).length);
      return acc;
    ...
    XLSX.writeFile(workbook, `Funcionarios_Export_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (!rawData || rawData.length === 0) {
          onToast("O arquivo Excel correspondente está vazio ou em formato inválido.", "error");
          return;
        }
        ...
        const colNome = keys.find(k => k.toLowerCase().includes('nome') || k.toLowerCase().includes('funcionario') || k.toLowerCase().includes('funcionário'));
        const colEmail = keys.find(k => k.toLowerCase().includes('email') || k.toLowerCase().includes('e-mail'));
        const colTipo = keys.find(k => k.toLowerCase().includes('tipo') || k.toLowerCase().includes('cargo'));
        const colMatricula = keys.find(k => k.toLowerCase().includes('matr') || k.toLowerCase().includes('id'));

        if (!colNome || !colMatricula) {
          onToast("Colunas 'Nome Completo' e 'Matrícula' não foram identificadas no Excel.", "error");
          return;
        }
        ...
        for (const row of rawData) {
          ...
          const rawTipoInput = colTipo ? String(row[colTipo] || '').trim().toLowerCase() : 'docente';
          
          const finalTipo: 'docente' | 'administrativo' = 
            (rawTipoInput.includes('adm') || rawTipoInput.includes('setor') || rawTipoInput.includes('gerente') || rawTipoInput.includes('assistente'))
              ? 'administrativo' 
              : 'docente';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h3 className="text-xl font-bold text-slate-900">Gestão de Funcionários (Insumos)</h3>
          <p className="text-sm text-slate-500">
            Cadastre funcionários administrativos e docentes para puxar matrícula automaticamente em requisições de Controle de Insumos.
          </p>
        </div>
      ...
      {/* Cadastro Form */}
      <form onSubmit={handleAddFuncionario} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <h4 className="font-bold text-slate-800 text-sm">Cadastrar Novo Funcionário</h4>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500">Tipo de Funcionário</label>
            <select
              value={tipo}
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500">Matrícula (ID)</label>
            <input
              type="text"
              required
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-slate-400 text-sm">Nenhum funcionário encontrado.</div>
        ) : (
